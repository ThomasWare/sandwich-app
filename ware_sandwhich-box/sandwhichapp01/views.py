...
class SandwichappView(View):   #SandwichView class
 ...
 #context dictionary meats, cheeses, and toppings are the keys.           
ingredients = {
...
class IngredientsListView(View): #class=based view inherited from View class
    def get(self, request, ingredient_type): # get function
        if request.method == 'GET':
...
class SandwichGeneratorView(View):   #SandwichGenerator class=based view
    def get(self, request):
        if request.method == 'GET':
